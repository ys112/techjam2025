...
class FastEmbeddingClassifier:
    ...
    def preprocess_metadata(self, df: pd.DataFrame) -> np.ndarray:
        ...
        if "text" in df.columns:
            ...
            metadata_features.extend(
                [text_lengths.values.reshape(-1, 1), word_counts.values.reshape(-1, 1)]
            )
    ...
    def train_classifiers(
        ...
        for task, y_full in y_dict.items():
            ...
            grid_search = GridSearchCV(
                base_clf, param_grid, cv=cv, scoring="f1", n_jobs=-1, verbose=0
            )
    ...
    def get_feature_importance(
        ...
        all_feature_names = embedding_names + getattr(
...
def create_sample_dataset(size: int = 2000) -> pd.DataFrame:
    ...
    for i in range(size):
        ...
        review_type = np.random.choice(
            ["clean", "spam", "ad", "rant"], p=[0.6, 0.15, 0.15, 0.1]
        )
    ...
    print(
        f"   Advertisements: {df['is_advertisements'].sum()} ({df['is_advertisements'].mean():.1%})"
    )
    print(
        f"   Rant without visit: {df['is_rant_without_visit'].sum()} ({df['is_rant_without_visit'].mean():.1%})"
    )
...
def plot_results(results: Dict[str, Dict]) -> None:
    ...
    avg_metrics = [
        np.mean([results[task][metric] for task in tasks]) for metric in metrics
    ]
